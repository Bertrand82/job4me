...
export class BgIndexedDBService {
  ...
  ajouterArchive(offre: ArchiveLettre) {
    ...
    const obs: Observable<ArchiveLettre & WithID> = this.dbService.add(
      'offreEmploi',
      offre
    );
  ...
  supprimerArchive(id: number) {
    const obs: Observable<unknown[]> = this.dbService.delete('offreEmploi', id);
    obs.subscribe({
      next: () => {
        console.log(
          "bg XXXX  IndexedDB - suppression de l'offre : success ",
          id
        );
        //this.testArchives();
      },
      error: (error) => {
        console.error(
          "bg YYYY  IndexedDB - suppression de l'offre : error ",
          error
        );
      },
    });
  ...
  getArchives(): Promise<ArchiveLettre[]> {
    const promise: Promise<ArchiveLettre[]> = firstValueFrom(
      this.dbService.getAll('offreEmploi')
    );
    return promise;
  }
  ...
  testArchives() {
    this.getArchives()
      .then((offres) => {
        console.log(
          'bg EEEE  IndexedDB - récupération des offres : success ',
          offres
        );
      })
      .catch((error) => {
        console.error(
          'bg FFFF  IndexedDB - récupération des offres : error ',
          error
        );
      });
  }
  ...
  testCVs() {
    this.getCVs()
      .then((listCv) => {
        console.log(
          'bg EEEE  IndexedDB - récupération des cv  : success ',
          listCv
        );
      })
      .catch((error) => {
        console.error(
          'bg FFFF  IndexedDB - récupération des cv : error ',
          error
        );
      });
  }
  ...
  ajouterCV(cvItem: CV) {
    console.log('bg AAAA1111 IndexedDB - ajout du CV : ', cvItem);
    const obs: Observable<CV & WithID> = this.dbService.add('CV', cvItem);
    console.log('bg AAAA2222 IndexedDB - ajout du CV : ', cvItem);
  ...
  updateCV(cvItem: CV) {
    console.log('bg AAAA1111 IndexedDB - ajout du CV : ', cvItem);
    const obs: Observable<CV & WithID> = this.dbService.update('CV', cvItem);
    console.log('bg AAAA2222 IndexedDB - ajout du CV : ', cvItem);
  ...
  supprimerCV(id: number) {
    const obs: Observable<unknown[]> = this.dbService.delete('CV', id);
    obs.subscribe({
      next: () => {
        console.log('bg XXXX  IndexedDB - suppression du CV : success ', id);
        //this.testArchives();
      },
      error: (error) => {
        console.error('bg YYYY  IndexedDB - suppression du CV : error ', error);
      },
    });
  }
  getCVs(): Promise<CV[]> {
