...
import {
  ...
  StripeSession,
  BgBackFunctions,
} from 'bg-navigation-auth-stripe';
...
export class ComponentStripe {
  ...
  mettreAJourInfosStripeCustomer() {
    this.http
      .get<{
        ...
        has_more: boolean;
        url: string;
      }>(`${this.baseUrl0}/bgstripegetclient2?email=` + this.getEmail(), {})
      .subscribe({
        next: (res) => {
  ...
  mettreAJourInfosStripePayments(): void {
    console.log(
      'mettreAJourInfosStripePayments start',
      this.bgAuth.stripeCustomer
    );
    ...
    if (!stripeCustomerId) {
      ...
      this.searchStripeCustomerOrCreate(
        this.mettreAJourInfosStripePayments2.bind(this)
      );
    } else {
  ...
  private getStripeInvoiceFromPaymentIntent(paymentIntentId: string) {
    console.log(
      'getStripeInvoiceFromPaymentIntent pour le paymentIntentId :',
      paymentIntentId
    );
    this.http
      .get<any>(
        `${this.baseUrl0}/bgstripegetinvoicefrompaymentintent2?paymentIntentId=` +
          paymentIntentId +
          '&email=' +
          this.getEmail() +
          '',
      ...
      .subscribe({
        next: (res) => {
          console.log(
            'Response from bgstripegetinvoicefrompaymentintent2:',
            res
          );
        },
        error: (err) => {
          console.error(
            'Erreur from bgstripegetinvoicefrompaymentintent2:',
            err
          );
        },
      });
  ...
  resilierAbonnement2(stripeCustomerId: string) {
    console.log(
      'resilierAbonnement2 pour le stripeCustomerId :',
      stripeCustomerId
    );
    this.http
      .post<any>(`${this.baseUrl0}/bgstripegetsession2`, {
  ...
  */
  getStripePaymentLink(priceIdStripe: any) {
    ...
    if (
      ...
      console.error(
        'Paramètre manquant pour Stripe ! email :',
        this.getEmail()
      );
      console.error(
        'Paramètre manquant pour Stripe ! priceIdStripe :',
        priceIdStripe
      );
      console.error(
        'Paramètre manquant pour Stripe ! stripeCustomerId :',
        this.getStripeCustomerId()
      );
      return;
    }
    ...
    console.log(
      'Appel de la fonction cloud bgstripegetpaymentlink avec :',
      params.toString()
    );
    const url0 = `${
      this.baseUrl0
    }/bgstripegetpaymentlink2?${params.toString()}`;
  ...
  createStripeCustomer() {
    ...
    this.http
      .get<StripeCustomer>(
        `${this.baseUrl0}/bgstripecreatecustomer2?${params.toString()}`,
        {}
      )
      .subscribe({
        next: (res) => {
  ...
  searchStripeCustomerOrCreate(aCallBack?: (stripeCustomerId: string) => void) {
    ...
    this.http.get<any>(`${baseUrl}?${params.toString()}`, {}).subscribe({
      next: (res) => {
        console.log(
          'BG Response from bgstripesearchclientsbybguseridorcreateclient:',
          res
        );
        this.bgAuth.stripeCustomer = res;
        this.bgAuth.saveStripeCustomerInLocal();
        ...
        if (aCallBack) {
          ...
          console.log(
            'BG aCallBack getStripeCustomerId :',
            this.getStripeCustomerId()
          );
          const stripeCustomerId = this.getStripeCustomerId();
          if (stripeCustomerId) {
            ...
            console.error(
              'BG Erreur: pas de stripeCustomerId après création ou recherche du client'
            );
          }
        }
      },
      error: (err) => {
        console.error(
          'Erreur from bgstripesearchclientsbybguseridorcreateclient:',
          err
        );
      },
    });
  ...
  searchCustomersByBgUserId(suite?: () => void) {
    const baseUrl = `${this.baseUrl0}/bgstripesearchclientsbybguserid2`;
  ...
  searchStripeSessionsByBgUserId() {
    ...
    if (!stripeCustomerId) {
      console.log('Aucun client Stripe trouvé pour cet utilisateur.');
      this.searchStripeCustomerOrCreate(
        this.searchStripeSessionsByBgUserId2.bind(this)
      );
    } else {
  ...
  searchStripeSessionsByBgUserId2(stripeCustomerId: string) {
    console.log(
      'searchStripeSessionsByBgUserId2 called with:',
      stripeCustomerId
    );
    ...
    this.http.get<any>(`${baseUrl}?${params.toString()}`, {}).subscribe({
      next: (res) => {
        ...
        console.log('componentStripe listSessions:', listSessions);
        if (listSessions.length > 0) {
          console.log('componentStripe Sessions Stripe trouvées pour ce client.');
          for (const session of listSessions) {
            ...
            const date = new Date(session.expires_at * 1000); // Multiplie par 1000 pour convertir en millisecondes // const date = new Date(session.ended_at * 1000); // Multiplie par 1000 pour convertir en millisecondes
            console.log("session expires_at:", date.toLocaleString());
          }
        }
      },
      error: (err) => {
  ...
  getStripeInvoiceById(invoiceId: any) {
    ...
    this.http
      .get<StripeInvoice>(
        `${this.baseUrl0}/bgstripegetinvoicebyid2?invoiceId=` + invoiceId,
        {}
      )
      .subscribe({
        next: (res) => {
  ...
  getStripeSubscriptionById() {
    const subscriptionId = this.bgAuth.stripeSession?.subscription;
    console.log(
      'getStripeSubscriptionById pour le subscriptionId :',
      subscriptionId
    );
    if (subscriptionId) {
      this.bgAuth.fetchStripeSubscriptionById();
    } else {
      console.error('Aucun subscriptionId trouvé dans la session Stripe.');
    }
  }
  ...
  getStripeSessionExpireAt() {
    const stripeSession = this.bgAuth.stripeSession;
    if (stripeSession && typeof stripeSession.expires_at === 'number') {
      const date = new Date(stripeSession.expires_at * 1000); // Convertir en millisecondes.current_period_end * 1000);
